...
- хотя реально занята меньше трети. Смысл именно в запасе на рост
(больше источников света, текстуры) без повторной перенумерации;
неиспользуемые смещения просто читаются как 0 и игнорируют запись.

Пока без текстур - цвет закреплён за каждой вершиной (Gouraud shading,
как в PS1/N64), растеризатор линейно интерполирует цвет и глубину между
тремя вершинами треугольника. `VU`/`VV`/`VTEXTURE` (смещения 12-14) -
**зарезервированные** поля под будущий протокол текстур - пишутся и
читаются как обычные регистры, но рендерингом пока игнорируются. Цвет,
переданный через `VR`/`VG`/`VB`, - это БАЗОВЫЙ цвет треугольника: если
перед `SUBMIT_VERTEX` задана нормаль (`VNX`/`VNY`/`VNZ`), реально
сохранённый цвет этой вершины - базовый цвет, промодулированный светом
(см. ниже); вершина без заданной нормали (по умолчанию все нули) не
получает направленного освещения, только вклад ambient-уровня.
...
| 6 | VR | цвет вершины/куба, R (0-255) |
| 7 | VG | цвет вершины/куба, G |
| 8 | VB | цвет вершины/куба, B |
| 9-11 | VNX/VNY/VNZ | нормаль СЛЕДУЮЩЕЙ вершины (см. "Освещение" ниже) |
| 12 | VU | **зарезервировано** - текстурная U (не используется) |
| 13 | VV | **зарезервировано** - текстурная V (не используется) |
| 14 | VTEXTURE | **зарезервировано** - id текстуры (не используется) |
...
- **5 (DRAW_CUBE)** - аппаратный быстрый путь для очень частого случая
  ...
  растеризует их через ТОТ ЖЕ пайплайн, что и `DRAW_TRIANGLE` - та же
  трансформация модели/камеры, то же освещение, тот же z-буфер. Не
  трогает `pendingVertices` вообще - можно мешать `DRAW_CUBE` и ручные
