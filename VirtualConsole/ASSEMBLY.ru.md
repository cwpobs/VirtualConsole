...
вместо побайтной записи имени файла устройства.
...
`VIDEOCARD_*` (фон/спрайты/скролл), `PNGLOADER_*`, `MAPLOADER_*`, плюс
готовые маппированные массивы `pngLoaderName`/`mapLoaderName`. Полный
список - см. "Встроенные константы регистров устройств (прелюдия)"
выше.
...
переиспользуемая библиотечная функция).