...
| 0xF0001044 | неиспользуемая дыра (старое место SoundCard - см. ниже) |
...
скролл), `ConsoleLayer`, `PngLoader`, `MapLoader` (что делает каждый
регистр - см. их разделы выше). Объявлять их самому не нужно - они уже
есть, с префиксом устройства в имени, чтобы одинаковые имена регистров
у разных устройств (`COMMAND`, `STATUS`) не конфликтовали:
...
Gpu3D - исключение: его регистры НЕ в прелюдии (см. пример
`C/DEMOS/SNOW3D.MC`, где они объявлены вручную как обычные `const`) -
адреса стабильны, при желании их несложно добавить в прелюдию так же,
как остальные, но пока это не сделано.
...
`MAPLOADER_*`, `EXEC_CHILD_DISK`, `CMD_ARGS_LEN`, плюс готовые
маппированные массивы `pngLoaderName`/`mapLoaderName`/`cmdArgs`.
Полный список - см. "Встроенные константы регистров устройств
(прелюдия)" выше.
