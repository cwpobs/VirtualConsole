...
| 0xF0001100 - 0xF00011FF | Gpu3D (3D-ускоритель + освещение + математический сопроцессор 16/32 бита - вершины/треугольники/кубы/PRESENT; зарезервировано 256 Б, используется ~78) |
...
пример (сетка 8x8, значения 0-2 - см. `C/DEMOS/CUBEWORLD.MC`, который
...
`MatrixLoader` (что делает каждый регистр - см. их разделы выше).
...
бита, см. его раздел выше) - исключение: его регистры НЕ в прелюдии, они
объявлены вручную как обычные `const` в библиотеке `C/DEV/LIB/GEOM3D.MC`
(`#include "GEOM3D.MC"` - см. "#include" ниже) - адреса стабильны, при
желании их несложно добавить в прелюдию так же, как остальные, но это
сознательно не сделано, чтобы прелюдия оставалась компактной и покрывала
устройства, нужные почти любой программе, а не специфичные для 3D.
...
`MAPLOADER_*`, `MOUSE_*`, `MATRIXLOADER_*`, `EXEC_CHILD_DISK`,
`CMD_ARGS_LEN`, плюс готовые маппированные массивы `pngLoaderName`/
`mapLoaderName`/`matrixLoaderName`/`cmdArgs`. `GPU3D_*` (3D + освещение
+ математический сопроцессор) в прелюдии НЕТ - вместо этого
`#include "GEOM3D.MC"` (см. "#include" ниже). Полный список - см.
