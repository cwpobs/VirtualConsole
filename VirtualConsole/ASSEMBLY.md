...
Пример - запустить уже загруженную песню и остановить по ESC (полный
рабочий пример - `C/MUSIC.ASM`, `exec music.asm`):
...
Читает и разбирает трекерный `.mod`-файл с диска "C" (без
поддиректорий) на стороне C++ и кладёт готовую песню в `SoundCard`
одним вызовом. MOD - бинарный формат с собственным секвенсором
...
Полный рабочий пример - `C/MUSIC.ASM` (запускается `exec music.asm`),
песня - `C/space_debris.mod`.
...
со скроллом) + `PngLoader`/`MapLoader` для видео; `SoundCard` (MOD-
проигрыватель) + `ModLoader` для звука. См. раздел 9.