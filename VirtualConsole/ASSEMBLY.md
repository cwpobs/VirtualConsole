...
отличает их от прямого `poke()`. Реализовано через ту же идею, что
`goto_row_start` в `SHELL.ASM`: `y*80` считается не одним `MUL`
(переполнился бы уже при `y >= 4`), а циклом прибавлений по 80 через
`__mc_hladd` - компилятор сам добавляет этот код (`__mc_screen_offset`)
в сгенерированную программу, если хоть раз встретил одну из этих
функций.
