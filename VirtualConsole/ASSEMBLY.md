...
| 0xF006 | CONTROL | Бит 0 = ENABLE, бит 1 = PENDING, бит 2 = HAS_DATA (очередь не пуста) | Бит 0 записанного значения = ENABLE; любая запись сбрасывает PENDING |
...
Косвенной/индексной адресации у процессора нет, поэтому адрес
каждого символа приходится указывать как отдельное число —
CALL/цикл с вычислением адреса на лету не сработает.

Пример вывода "Hi" в левый верхний угол экрана:
