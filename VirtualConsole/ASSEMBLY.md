...
Грузит PNG-файл с диска "C", из ТЕКУЩЕЙ папки диска (`Disk::
getCurrentPath()` - той же, что меняет команда `cd`, и которую уже
использует `exec`/`type`/`dir`) - поэтому запущенная из `C:\DEMOS`
программа (см. `C/DEMOS/*.ASM`) находит свои же ресурсы рядом с
собой, без специального механизма "программа знает свой путь". И
вырезает из него квадраты 32x32 прямо в
...
Грузит текстовый файл тайловой карты с диска "C", из ТЕКУЩЕЙ папки
диска (см. `PngLoader` выше про `Disk::getCurrentPath()`), и кладёт
его прямо в `VideoCard` (см. "Аппаратные тайлы и скролл" выше) - какой
...
Читает и разбирает трекерный `.mod`-файл с диска "C", из ТЕКУЩЕЙ папки
диска (см. `PngLoader` выше про `Disk::getCurrentPath()`), на стороне
C++ и кладёт готовую песню в `SoundCard` одним вызовом. MOD - бинарный
