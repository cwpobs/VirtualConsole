...
Помимо `ИМЯ.RUN`, `BUILD` всегда пишет рядом `ИМЯ.REL` - таблицу
релокаций (см. `Assembler::assemble()`, третий необязательный
параметр `relocations`): список смещений (относительно начала
машинного кода) всех 4-байтных адресов, которые ассемблер получил из
МЕТКИ (`JMP`/`CALL`/`LDA`/`STA`/`LDHL` на свою же метку/переменную
программы), а не из числового литерала. Формат - 4 байта (количество
смещений, `uint32` LE), затем сами смещения по 4 байта (`uint32` LE).
Нужна, чтобы этот `.RUN` можно было безопасно запустить НЕ там, где он
собран - см. `LOAD_CHILD` ниже. Числовые литералы вроде
`STA 0xF00007D8` (абсолютные адреса устройств) в таблицу не попадают -
они не зависят от того, где лежит сама программа, сдвигать их нельзя.
...
В отличие от `LOAD_RAW`, простого побайтового копирования тут
недостаточно: `ИМЯ.RUN` собран из предположения, что будет выполняться
по `0x00002000` - все его собственные `JMP`/`CALL`/`LDA`/`STA` на свои
же метки/переменные это отражают как абсолютные адреса. `LOAD_CHILD`
читает `ИМЯ.REL` (см. `BUILD` выше), для каждого записанного там
смещения сдвигает соответствующий 4-байтный адрес на разницу между
адресом уровня и `0x00002000`, и только потом пишет пропатченные байты
в RAM. Если `ИМЯ.REL` не найден (`.RUN` собран до появления этого
механизма, либо `.REL` удалён отдельно) - `STATUS=2`, отказ (нужно
пересобрать через `BUILD`) - угадывать тут не пытаемся.
