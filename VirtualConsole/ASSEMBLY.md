...
| Адрес | Регистр | Чтение | Запись |
|-------|---------|--------|--------|
| 0xF0000FF8 | MILLIS_LOW | Младший байт прошедших мс | Сбрасывает часы (обнуляет прошедшее время) |
| 0xF0000FF9 | MILLIS_HIGH | Старший байт прошедших мс | Сбрасывает часы |
...
Пример - ждать ровно 60 кадров в секунду (~16.7 мс на кадр):
    ...
    LDI A, 0
    STA 0xF0000FF8      ; сброс часов

wait_frame:

    LDA 0xF0000FF9      ; MILLIS_HIGH - страховка на случай >255 мс
    LDI B, 0
    CMP B
    JNZ frame_done

    LDA 0xF0000FF8      ; MILLIS_LOW
    LDI B, 17
    CMP B
    JC wait_frame
...
Адреса: `0xF0000FDE` (X_LOW) - `0xF0000FF7` (SPRITE_STATUS) - весь
блок `VideoCard` целиком, фон и спрайты вместе.
...
видеопамять аппаратных спрайтов `VideoCard`. PNG - сжатый формат
(zlib/deflate) - декодировать его в ассемблере этого CPU нереально,
поэтому весь разбор формата (через `stb_image.h`) целиком спрятан на
стороне C++: ассемблеру видны только команды "загрузи файл" и "вырежи
регион в спрайт N".
...
| 16 | SPRITE_INDEX | номер спрайта-приёмника у `VideoCard` (0-15) |
| 17 | COMMAND (запись = триггер) | 1=LOAD, 2=EXTRACT |
| 18 | STATUS (только чтение) | 0=ok, 1=файл не найден/не декодировался, 2=регион вне картинки, 3=неверный SPRITE_INDEX |

Адреса: `0xF0000FFA` (NAME0) - `0xF000100C` (STATUS).

Команды:

- **1 (LOAD)** - декодирует файл `NAME` в RGBA и кэширует результат
  внутри устройства (ширина/высота/пиксели) до следующего `LOAD` -
  один `LOAD` спрайтшита обслуживает много последующих `EXTRACT` без
  повторного декодирования.
- **2 (EXTRACT)** - вырезает квадрат 32x32 из закэшированной картинки
  ...
  копируется RGB как есть - модель спрайтов не хранит альфу, только
  chroma-key, см. "Аппаратные спрайты" выше) и пишет результат в
  спрайт `SPRITE_INDEX`.
...
Пример - загрузить спрайтшит и положить ячейку `(64, 0)` в спрайт 2:
    ...
    LDI A, 1
    STA 0xF000100B      ; COMMAND = LOAD

    LDA 0xF000100C       ; STATUS - проверить перед EXTRACT
    LDI B, 0
    CMP B
    JNZ png_error

    LDI A, 64
    STA 0xF0001006      ; SRC_X_LOW = 64
    LDI A, 0
    STA 0xF0001007      ; SRC_X_HIGH = 0
    STA 0xF0001008      ; SRC_Y_LOW = 0
    STA 0xF0001009      ; SRC_Y_HIGH = 0
    LDI A, 2
    STA 0xF000100A      ; SPRITE_INDEX = 2
    LDI A, 2
    STA 0xF000100B      ; COMMAND = EXTRACT
